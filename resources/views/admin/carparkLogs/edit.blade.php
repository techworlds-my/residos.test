...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.carparkLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.carpark-logs.update", [$carparkLog->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="time_in">{{ trans('cruds.carparkLog.fields.time_in') }}</label>
                <input class="form-control {{ $errors->has('time_in') ? 'is-invalid' : '' }}" type="text" name="time_in" id="time_in" value="{{ old('time_in', $carparkLog->time_in) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time_out') ? 'is-invalid' : '' }}" type="text" name="time_out" id="time_out" value="{{ old('time_out', $carparkLog->time_out) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $carparkLog->price) }}" step="1">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carplate') ? 'is-invalid' : '' }}" name="carplate_id" id="carplate_id" required>
                    @foreach($carplates as $id => $carplate)
                        <option value="{{ $id }}" {{ (old('carplate_id') ? old('carplate_id') : $carparkLog->carplate->id ?? '') == $id ? 'selected' : '' }}>{{ $carplate }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id" id="location_id">
                    @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $carparkLog->location->id ?? '') == $id ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
