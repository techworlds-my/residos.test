...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.carparkPayment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.carpark-payments.update", [$carparkPayment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="carplate">{{ trans('cruds.carparkPayment.fields.carplate') }}</label>
                <input class="form-control {{ $errors->has('carplate') ? 'is-invalid' : '' }}" type="text" name="carplate" id="carplate" value="{{ old('carplate', $carparkPayment->carplate) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment') ? 'is-invalid' : '' }}" type="number" name="payment" id="payment" value="{{ old('payment', $carparkPayment->payment) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" type="text" name="payment_method" id="payment_method" value="{{ old('payment_method', $carparkPayment->payment_method) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remark') ? 'is-invalid' : '' }}" type="text" name="remark" id="remark" value="{{ old('remark', $carparkPayment->remark) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $carparkPayment->status) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id" id="location_id" required>
                    @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $carparkPayment->location->id ?? '') == $id ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
