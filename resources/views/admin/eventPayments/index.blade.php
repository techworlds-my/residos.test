...
@can('event_payment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.event-payments.create') }}">
                        ...
                        <tr data-entry-id="{{ $eventPayment->id }}">
                            ...
                            <td>
                                @can('event_payment_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.event-payments.show', $eventPayment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('event_payment_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.event-payments.edit', $eventPayment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('event_payment_delete')
                                    <form action="{{ route('admin.event-payments.destroy', $eventPayment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('event_payment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.event-payments.massDestroy') }}",
