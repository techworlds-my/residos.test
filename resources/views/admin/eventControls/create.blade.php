...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.eventControl.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-controls.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('participants') ? 'is-invalid' : '' }}" type="number" name="participants" id="participants" value="{{ old('participants', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('audience_group') ? 'is-invalid' : '' }}" name="audience_group_id" id="audience_group_id">
                    @foreach($audience_groups as $id => $audience_group)
                        <option value="{{ $id }}" {{ old('audience_group_id') == $id ? 'selected' : '' }}>{{ $audience_group }}</option>
                    @endforeach
                </select>
