...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.defactCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.defact-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="defact_category">{{ trans('cruds.defactCategory.fields.defact_category') }}</label>
                <input class="form-control {{ $errors->has('defact_category') ? 'is-invalid' : '' }}" type="text" name="defact_category" id="defact_category" value="{{ old('defact_category', '') }}" required>
