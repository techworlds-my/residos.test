...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.entrance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.entrances.update", [$entrance->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.entrance.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $entrance->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('last_active') ? 'is-invalid' : '' }}" type="text" name="last_active" id="last_active" value="{{ old('last_active', $entrance->last_active) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('in_enable') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="in_enable" id="in_enable" value="1" {{ $entrance->in_enable || old('in_enable', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="in_enable">{{ trans('cruds.entrance.fields.in_enable') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id" id="location_id" required>
                    @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $entrance->location->id ?? '') == $id ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
