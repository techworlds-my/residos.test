...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.unit-managements.update", [$unitManagement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="number" name="size" id="size" value="{{ old('size', $unitManagement->size) }}" step="1" required>
                @if($errors->has('size'))
                    <div class="invalid-feedback">
                        {{ $errors->first('size') }}
                    </div>
