        ...
        <div class="col-md-12">
            @can('feedback_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.feedback-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $feedbackCategory->id }}">
                                        ...
                                        <td>
                                            @can('feedback_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.feedback-categories.show', $feedbackCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('feedback_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.feedback-categories.edit', $feedbackCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feedback_category_delete')
                                                <form action="{{ route('frontend.feedback-categories.destroy', $feedbackCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feedback_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.feedback-categories.massDestroy') }}",
