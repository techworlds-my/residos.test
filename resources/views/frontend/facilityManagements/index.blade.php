@extends('layouts.frontend')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            @can('facility_management_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.facility-managements.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.facilityManagement.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'FacilityManagement', 'route' => 'admin.facility-managements.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                <div class="card-header">
                    {{ trans('cruds.facilityManagement.title_singular') }} {{ trans('global.list') }}
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-FacilityManagement">
                            <thead>
                                <tr>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.id') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.status') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.name') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.image') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.category') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.open') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.facilityManagement.fields.closed') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($facilityManagements as $key => $facilityManagement)
                                    <tr data-entry-id="{{ $facilityManagement->id }}">
                                        <td>
                                            {{ $facilityManagement->id ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\FacilityManagement::STATUS_SELECT[$facilityManagement->status] ?? '' }}
                                        </td>
                                        <td>
                                            {{ $facilityManagement->name ?? '' }}
                                        </td>
                                        <td>
                                            @if($facilityManagement->image)
                                                <a href="{{ $facilityManagement->image->getUrl() }}" target="_blank" style="display: inline-block">
                                                    <img src="{{ $facilityManagement->image->getUrl('thumb') }}">
                                                </a>
                                            @endif
                                        </td>
                                        <td>
                                            {{ $facilityManagement->category->category ?? '' }}
                                        </td>
                                        <td>
                                            {{ $facilityManagement->open ?? '' }}
                                        </td>
                                        <td>
                                            {{ $facilityManagement->closed ?? '' }}
                                        </td>
                                        <td>
                                            @can('facility_management_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.facility-managements.show', $facilityManagement->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('facility_management_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.facility-managements.edit', $facilityManagement->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('facility_management_delete')
                                                <form action="{{ route('frontend.facility-managements.destroy', $facilityManagement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                                    <input type="hidden" name="_method" value="DELETE">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                    <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                                </form>
                                            @endcan

                                        </td>

                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('facility_management_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('frontend.facility-managements.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-FacilityManagement:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection