        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.checkFacility.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.check-facilities.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="defect_id" id="defect_id">
                                @foreach($defects as $id => $defect)
                                    <option value="{{ $id }}" {{ old('defect_id') == $id ? 'selected' : '' }}>{{ $defect }}</option>
                                @endforeach
                            </select>
