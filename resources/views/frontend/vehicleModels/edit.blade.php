    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.vehicle-models.update", [$vehicleModel->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="modal">{{ trans('cruds.vehicleModel.fields.modal') }}</label>
                            <input class="form-control" type="text" name="modal" id="modal" value="{{ old('modal', $vehicleModel->modal) }}" required>
                            @if($errors->has('modal'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="model" id="model" value="{{ old('model', $vehicleModel->model) }}" required>
                            @if($errors->has('model'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('model') }}
                                </div>
                        ...
                        <div class="form-group">
                            <div>
                                <input type="checkbox" name="is_enable" id="is_enable" value="1" {{ $vehicleModel->is_enable || old('is_enable', 0) === 1 ? 'checked' : '' }} required>
